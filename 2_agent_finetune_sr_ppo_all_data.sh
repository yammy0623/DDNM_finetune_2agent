...
ALG="PPO"
STEPS=5
SAVEFOLDER=$DEG"_imagenet_2_agents_"$ALG"_"$STEPS"_alldata"
# Deblur
...
# train ours (1st subtask)
python train.py --ni --config imagenet_256.yml --exp $EXP --path_y imagenet --eta 0.85 --deg $DEG --deg_scale 4 --sigma_y 0. -i "imagenet_"$ALG"_"$STEPS"_"  --target_steps $STEPS --seed $SEED --save_path "./model/"$SAVEFOLDER
# eval ours (1st subtask)
python eval.py --ni --config imagenet_256.yml --exp $EXP --path_y imagenet --eta 0.85 --deg $DEG  --deg_scale 4 --sigma_y 0. -i "imagenet_"$ALG"_"$STEPS"_eval" --target_steps $STEPS --eval_model_name "$DEG"_imagenet_2_agents_"$ALG"_"$STEPS" --save_path "./model/"$SAVEFOLDER --subtask1 >> model/"$SAVEFOLDER"/subtask1.txt
