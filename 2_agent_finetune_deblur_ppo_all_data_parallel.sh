...
SAVEFOLDER=$DEG"_imagenet_2_agents_"$ALG"_"$STEPS"_s1_disc_parallel"
ID=$DEG"_"$DATASET"_"$MODE"_"$ALG"_env_"$NUM_TRAIN_ENV"_steps_"$STEPS"_s1_disc_parallel"
...
# train ours (1st subtask)
python train_2.py --ni --config imagenet_256.yml --exp $EXP --path_y imagenet --eta 0.85 --deg $DEG --sigma_y 0. -i imagenet_"$ALG"_5_  --target_steps 5 --seed $SEED --save_path "./model/"$SAVEFOLDER --id $ID
