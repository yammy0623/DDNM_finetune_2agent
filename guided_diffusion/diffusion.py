...
class Diffusion(object):
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        print(f'Train dataset has size {len(dataset)}')
        ...
        for x_orig, classes in pbar:
            ...
            # A_inv_y = A_funcs.A_pinv(y.reshape(y.size(0), -1)).reshape(Apy[i].size())
            # x = A_inv_y.reshape(x.size())
            with torch.no_grad():
            ...
            for j in range(x[0].size(0)):
                ...
                per_ssim = structural_similarity(x[0][j].cpu().numpy(), orig.cpu().numpy(), win_size=21, channel_axis=0, data_range=1.0)
                avg_ssim += per_ssim
                print("Per PSNR: %.2f | Per SSIM: %.3f" % (psnr, per_ssim))


            idx_so_far += y.shape[0]

            pbar.set_description("PSNR: %.2f | SSIM: %.3f" % (avg_psnr / (idx_so_far - idx_init), avg_ssim / (idx_so_far - idx_init)))
