...
for Iter in {1..5}; do
    echo "Iteration: $Iter"
    python train.py --ni --config imagenet_256.yml --exp $EXP --path_y imagenet --eta 0.85 --deg "inpainting" --sigma_y 0. -i imagenet_inpainting --target_steps 5 --finetune 1
    # eval ours (1st subtask)
    ...
    # # train ours (2nd subtask)
    python train.py --ni --config imagenet_256.yml --exp $EXP --path_y imagenet --eta 0.85 --deg "inpainting" --sigma_y 0. -i imagenet_inpainting --target_steps 5 --second_stage --finetune 1
    # # eval ours
